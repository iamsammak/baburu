...
/***/ function(module, exports, __webpack_require__) {
	...
	var _cannonball = __webpack_require__(6);
	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    key: 'add',
	    value: function add(incomingObj) {
	      // debugger;
	      if (incomingObj instanceof _bubble2.default) {
	        this.bubbles.push(incomingObj);
	      } else if (incomingObj instanceof _cannonball2.default) {
	        this.cannonballs.push(incomingObj);
	      } else if (incomingObj instanceof _cannon2.default) {
	    ...
	    key: 'allThingsOnBoard',
	    value: function allThingsOnBoard() {
	      return [].concat(this.cannons, this.bubbles, this.cannonballs);
	    }
	  }, {
	    key: 'allMoveableObjects',
	    value: function allMoveableObjects() {
	    ...
	    key: 'overlapOnCreation',
	    value: function overlapOnCreation(bubble) {
	      var overlapped = false;
	      for (var i = 0; i < this.allThingsOnBoard().length; i++) {
	    ...
	    value: function checkCollisions() {
	      ...
	      var allObjects = this.allMoveableObjects(); //then everything just flies over cannon
	      for (var i = 0; i < allObjects.length; i++) {
	        for (var j = 0; j < allObjects.length; j++) {
	          var object1 = allObjects[i];
	          var object2 = allObjects[j];
	          if (object1.iscollidedWith(object2)) {
	            var collision = object1.collideWith(object2);
	            // debugger;
	            if (collision) return;
	            _physics2.default.collide(object1, object2);
	          }
	        }
	      }
	    }
	    ...
	    value: function draw(ctx) {
	      ...
	      this.allThingsOnBoard().forEach(function (object) {
	        object.draw(ctx);
	      });
	    }
	  }, {
	    ...
	    value: function moveObjects(delta) {
	      this.allMoveableObjects().forEach(function (object) {
	        object.move(delta);
	      });
	    }
	  }, {
	    ...
	    value: function remove(object) {
	      ...
	      } else if (object instanceof _cannonball2.default) {
	        this.cannonballs.splice(this.cannonballs.indexOf(object), 1);
	      } else if (object instanceof _cannon2.default) {
	    ...
	    key: 'checkCannonballs',
	    value: function checkCannonballs() {
	      var _this = this;
	    ...
	    value: function wrap(pos) {
	      return [_physics2.default.wrap(pos[0], this.width), _physics2.default.wrap(pos[1], this.height)];
	    }
	  }, {
	    key: 'isGameOver',
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Bubble = function (_MovingObject) {
	  ...
	  _createClass(Bubble, [{
	    key: 'bounce',
	    value: function bounce(x, y, otherObject) {
	      this.vel = [x, y];
	    ...
	    value: function collideWith(otherObject) {
	      if (otherObject instanceof _cannon2.default) {
	        // replace with bounce
	        // otherObject.relocate();
	        console.log("collideWith Cannon");
	        return true;
	      } else if (otherObject instanceof _cannonball2.default) {
...
/***/ function(module, exports) {
	...
	var Physics = function () {
	  ...
	  _createClass(Physics, null, [{
	    ...
	    key: 'scale',
	    value: function scale(vec, magnitude) {
	    ...
	    value: function collide(obj1, obj2) {
	      // calculate mass
	      var m1 = Math.PI * Math.pow(obj1.radius, 2);
	      var m2 = Math.PI * Math.pow(obj2.radius, 2);
	      ...
	      var mag1 = this.magnitude(obj1.vel);
	      var mag2 = this.magnitude(obj2.vel);
	      var d1 = Math.atan2(obj1.vel[1], obj1.vel[0]);
	      var d2 = Math.atan2(obj2.vel[1], obj2.vel[0]);
	
	      // calculate new speeds based on magnitude, direction and the collision angle
	      var newX1 = mag1 * Math.cos(d1 - angle);
	      var newY1 = mag1 * Math.sin(d1 - angle);
	      var newX2 = mag2 * Math.cos(d2 - angle);
	      var newY2 = mag2 * Math.cos(d2 - angle);
	      ...
	      var x1 = Math.cos(angle) * finalX1 + Math.cos(angle + Math.PI / 2) * finalY1;
	      var y1 = Math.sin(angle) * finalX1 + Math.sin(angle + Math.PI / 2) * finalY1;
	      var x2 = Math.cos(angle) * finalX2 + Math.cos(angle + Math.PI / 2) * finalY2;
	      var y2 = Math.sin(angle) * finalX2 + Math.sin(angle + Math.PI / 2) * finalY2;
	...
	var MovingObject = function () {
	  ...
	  _createClass(MovingObject, [{
	    ...
	    value: function iscollidedWith(otherObject) {
	      var distance = _physics2.default.dist(this.pos, otherObject.pos);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Cannon = function (_MovingObject) {
	  _inherits(Cannon, _MovingObject);
	  ...
	  _createClass(Cannon, [{
	    ...
	    value: function onMouseMove(e) {
	      ...
	      var mouseangle = _physics2.default.radToDeg(Math.atan2(this.y - pos.y, pos.x - this.x));
	      // debugger;
	    ...
	    value: function draw(ctx) {
	      ...
	      ctx.lineTo(this.pos[0] + 1.0 * this.game.level.tilewidth * Math.cos(_physics2.default.degToRad(this.angle)), this.pos[1] - 1.0 * this.game.level.tileheight * Math.sin(_physics2.default.degToRad(this.angle)));
	      ctx.stroke();
	    }
	  }, {
	    key: 'fireCannonball',
	    value: function fireCannonball() {
	      // const norm = Physics.norm(this.vel);
	      // let startVel = [0.1, -0.1];
	      var startVel = [Math.cos(_physics2.default.degToRad(this.angle)), -1 * Math.sin(_physics2.default.degToRad(this.angle))];
	
	      var relVel = _physics2.default.scale(_physics2.default.direction(startVel), _cannonball2.default.SPEED);
	      ...
	      var cannonballVel = [relVel[0] + startVel[0], relVel[1] + startVel[1]];
	
	      var cannonball = new _cannonball2.default({
	        pos: this.pos,
	        vel: cannonballVel,
	        color: this.color,
	        game: this.game
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Cannonball = function (_MovingObject) {
	  _inherits(Cannonball, _MovingObject);
	  ...
	  function Cannonball(options) {
	    ...
	    var _this = _possibleConstructorReturn(this, (Cannonball.__proto__ || Object.getPrototypeOf(Cannonball)).call(this, options));
	
	    _this.isWrappable = false;
	    _this.createdAt = _physics2.default.currentTime();
	    return _this;
	  }
